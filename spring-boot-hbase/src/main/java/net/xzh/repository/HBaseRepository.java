...
public class HBaseRepository {
    ...
    public boolean deleteDataByColumnFamily(String tableName, String columnFamily) {
        try {
            ...
            e.printStackTrace();
            log.debug(
                MessageFormat.format("删除该表中列簇下所有数据失败：表名：{0},列簇：{1}，错误信息：{2}", tableName, columnFamily, e.getMessage()));
            return false;
        }
    }
     ...
     *            列簇
     * @param queryParam
     *            过滤列集合 ("topicFileId,6282")=>("列,值")
     * @param regex
     *            分隔字符
     * @param bool
     *            查询方式：and 或 or | true : and ；false：or
     *
     * @return
     */
    ...
    public List<Map<String, Object>> selectColumnValueDataByFilter(String tableName, String columnFamily,
        ...
        for (String param : queryParam) {
            ...
            SingleColumnValueExcludeFilter singleColumnValueExcludeFilter =
                new SingleColumnValueExcludeFilter(Bytes.toBytes(columnFamily), Bytes.toBytes(queryArray[0]),
                    CompareOperator.EQUAL, Bytes.toBytes(queryArray[1]));
    ...
    private int queryDataCount(Table table, Scan scan) {
        ...
        try {
            ...
            log.info(
                MessageFormat.format("统计全表数据总数：表名：{0}，查询结果：{1}", Bytes.toString(table.getName().getName()), rowCount));
            return rowCount;
        } catch (Exception e) {
            e.printStackTrace();
            log.debug(MessageFormat.format("查询指定表中数据信息：表名：{0}，错误信息：{1}", Bytes.toString(table.getName().getName()),
