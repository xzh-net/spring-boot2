...
public class DirectExchangeConfig {
	...
	/**
	 * 创建直接交换机 直接交换机：根据精确的路由键匹配进行消息路由
	 */
	@Bean
	public DirectExchange directExchange() {
		return new DirectExchange(AMPQConstant.EXCHANGE_DIRECT);
	}

	/**
	 * 直接队列1 - 用于处理error和warn级别的日志 绑定路由键：error, warn
	 */
	@Bean
	...
	/**
	 * 直接队列2 - 用于处理info和debug级别的日志 绑定路由键：info, debug
	 */
	@Bean
	...
	/**
	 * 绑定队列1到直接交换机 - 警告级别路由 路由键：warn
	 */
	@Bean
	public Binding directBindingWarn(DirectExchange directExchange, Queue directQueue1) {
		return BindingBuilder.bind(directQueue1).to(directExchange).with(ROUTING_KEY_WARN);
	}

	/**
	 * 绑定队列2到直接交换机 - 信息级别路由 路由键：info
	 */
	@Bean
	...
	/**
	 * 绑定队列2到直接交换机 - 调试级别路由 路由键：debug
	 */
	@Bean
	public Binding directBindingDebug(DirectExchange directExchange, Queue directQueue2) {
		return BindingBuilder.bind(directQueue2).to(directExchange).with(ROUTING_KEY_DEBUG);
	}
}