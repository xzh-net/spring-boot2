 ...
 * 死信队列监听器
 * 
 * 死信队列典型应用场景： 1. 订单超时取消：订单创建后发送延迟消息，到期检查订单状态 2. 支付超时处理：发送支付消息，超时未支付则取消订单 3.
 * 消息重试机制：处理失败的消息进入死信队列，延迟重试 4. 定时任务：基于消息的延迟执行定时任务
 * 
 */
@Component
public class DeadLetterExchangeListener {
	 ...
	 * 监听死信接收队列 - 处理延迟到期的消息
	 * 
	 * 使用手动确认模式，确保消息处理成功后才确认 如果处理失败可以选择重新入队或进入死信队列的死信队列
	 * 
	...
	public void handleDeadLetterMessage(Message message, @Headers org.springframework.messaging.MessageHeaders headers,
	...
	private void processDeadLetterMessage(String messageBody, org.springframework.messaging.MessageHeaders headers) {
		...
		log.info("死信消息原始信息 - 交换机: {}, 路由键: {}, 死亡原因: {}", originalExchange, originalRoutingKey, deathReason);
	 ...
	 * @throws IOException 通道操作异常
	 */
	private void handleProcessingFailure(Message message, long deliveryTag, Channel channel, Exception exception)
			throws IOException {
		// 获取重试次数
		...
		if (retryCount < 3) {
			...
			// 或者可以发送到另一个专门的最终失败队列
			// channel.basicPublish("final.failure.exchange", "final.failure.key", null,
			// message.getBody());
		}
	}
