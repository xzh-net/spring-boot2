...
public class DelayedExchangeListener {
    ...
    public void handleDelayedMessage(Message message, @Headers Map<String, Object> headers, Channel channel) throws IOException {
        long deliveryTag = message.getMessageProperties().getDeliveryTag();
        String messageBody = new String(message.getBody(), StandardCharsets.UTF_8);
        
        try {
            log.info("延迟队列接收到消息, deliveryTag: {}, message: {}", 
                    deliveryTag, messageBody);
     ...
     * @throws IOException 通道操作异常
     */
    private void handleDelayedMessageFailure(Message message, long deliveryTag, Channel channel, Exception exception) throws IOException {
        // 获取重试次数
        ...
        if (retryCount < 3) {
            ...
            // 或者发送到专门的失败处理队列
            // channel.basicPublish("delayed.failure.exchange", "delayed.failure.key", null, message.getBody());
        }
    }

    /**
     * 延迟消息插件的优势场景：
     * 1. 精准定时任务：需要精确时间触发的业务
     * 2. 分级延迟：不同业务需要不同的延迟时间
     * 3. 动态延迟：根据业务逻辑动态计算延迟时间
     * 4. 大规模延迟任务：需要管理大量不同延迟时间的任务
     * 
     * 注意事项：
     * 1. 延迟时间不宜过长（建议不超过数天）
     * 2. 大量延迟消息可能占用较多内存
     * 3. 重启RabbitMQ可能导致未到期的延迟消息丢失（取决于持久化配置）
     */
}