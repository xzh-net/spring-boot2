...
public class DirectExchangeListener {
	...
	/**
	 * 监听直接队列1 - 处理error和warn级别的消息 路由键：[error, warn]
	 */
	@RabbitListener(queues = DirectExchangeConfig.DIRECT_QUEUE_1)
	public void handleErrorAndWarnMessages(String message) {
		log.info("{} - 队列1[{}|{}]接收到消息: {}", AMPQConstant.EXCHANGE_DIRECT, DirectExchangeConfig.ROUTING_KEY_ERROR,
				DirectExchangeConfig.ROUTING_KEY_WARN, message);
		// 这里可以添加业务处理逻辑
		processDirectMessage(message, "ERROR/WARN");
	}

	/**
	 * 监听直接队列2 - 处理info和debug级别的消息 路由键：[info, debug]
	 */
	@RabbitListener(queues = DirectExchangeConfig.DIRECT_QUEUE_2)
	public void handleInfoAndDebugMessages(String message) {
		log.info("{} - 队列2[{}|{}]接收到消息: {}", AMPQConstant.EXCHANGE_DIRECT, DirectExchangeConfig.ROUTING_KEY_INFO,
				DirectExchangeConfig.ROUTING_KEY_DEBUG, message);
		// 这里可以添加业务处理逻辑
