...
public class TopicExchangeListener {
	...
	/**
	 * 监听主题队列1 - 处理橙色相关和兔子相关的消息 路由模式：[*.orange.*, *.*.rabbit]
	 */
	@RabbitListener(queues = TopicExchangeConfig.TOPIC_QUEUE_1)
	public void handleOrangeAndRabbitMessages(String message) {
		log.info("{} - 队列1[{}|{}]接收到消息: {}", AMPQConstant.EXCHANGE_TOPIC, TopicExchangeConfig.ROUTING_PATTERN_ORANGE,
				TopicExchangeConfig.ROUTING_PATTERN_RABBIT, message);
		processTopicMessage(message, "队列1");
	}

	/**
	 * 监听主题队列2 - 处理懒惰相关的消息 路由模式：[lazy.#]
	 */
	@RabbitListener(queues = TopicExchangeConfig.TOPIC_QUEUE_2)
	public void handleLazyMessages(String message) {
		log.info("{} - 队列2[{}]接收到消息: {}", AMPQConstant.EXCHANGE_TOPIC, TopicExchangeConfig.ROUTING_PATTERN_LAZY,
				message);
